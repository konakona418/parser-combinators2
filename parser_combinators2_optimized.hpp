    ...
    namespace details {
        ...
        struct static_variant {
            ...
            constexpr auto get() -> T& {
                ...
                assert(type_index == idx && "Bad variant access in static_variant::get");
                return *reinterpret_cast<T*>(data);
            }

            template <size_t I>
            constexpr auto get_at_index() -> typename std::tuple_element_t<I, std::tuple<Ts...>>& {
                static_assert(I < sizeof...(Ts), "Index out of bounds in static_variant::get_at_index");
                assert(type_index == I && "Bad variant access in static_variant::get_at_index");
                using T = typename std::tuple_element_t<I, std::tuple<Ts...>>;
                return *reinterpret_cast<T*>(data);
            }

            constexpr void destroy() noexcept {
                if (type_index == 255) return;

                if constexpr (!(std::is_trivially_destructible_v<Ts> && ...)) {
                    template for (constexpr auto i : std::define_static_array(std::ranges::views::iota(0u, sizeof...(Ts)))) {
                        if (type_index == i) {
                            using T = typename std::tuple_element_t<i, std::tuple<Ts...>>;
            ...
            constexpr static_variant(static_variant&& other) noexcept {
                if (other.type_index == 255) return;

                if constexpr ((std::is_trivially_move_constructible_v<Ts> && ...)) {
                    std::memcpy(data, other.data, Size);
                } else {
                    template for (constexpr auto i : std::define_static_array(std::ranges::views::iota(0u, sizeof...(Ts)))) {
                        if (other.type_index == i) {
                            using T = typename std::tuple_element_t<i, std::tuple<Ts...>>;
                            new (data) T(std::move(*reinterpret_cast<T*>(other.data)));
                            type_index = static_cast<uint8_t>(i);
                            return;
                        }
                    }
                }
            ...
            constexpr static_variant& operator=(static_variant&& other) noexcept {
                if (this == &other) return *this;
                destroy();
                if (other.type_index == 255) return *this;
            ...
            template <typename T>
            requires (!std::is_same_v<std::remove_cvref_t<T>, static_variant>)
            constexpr static_variant(T&& val) noexcept {
                using RawT = std::remove_cvref_t<T>;
                new (data) RawT(std::forward<T>(val));
                type_index = static_cast<uint8_t>(index_of<RawT>());
            }

            template <size_t I, typename T>
            constexpr auto emplace(T&& val) {
                static_assert(I < sizeof...(Ts), "Index out of bounds in static_variant::emplace");
                using TargetT = typename std::tuple_element_t<I, std::tuple<Ts...>>;
                new (data) TargetT(std::forward<T>(val));
                type_index = static_cast<uint8_t>(I);
            }
            ...
            constexpr decltype(auto) visit(F&& f) {
                switch (type_index) {
                    template for (constexpr auto i : std::define_static_array(std::ranges::views::iota(0u, sizeof...(Ts)))) {
                        case i:
                            using CurrentT = typename std::tuple_element_t<i, std::tuple<Ts...>>;
                            return f(*reinterpret_cast<CurrentT*>(data));
                    }
        ...
        private:
            ...
            static consteval size_t index_of() {
                size_t i = 0;
                template for (constexpr auto t :  std::define_static_array(std::array{^^Ts...})) {
                    if (t == ^^T) {
                        return i;
                    }
                    ++i;
        ...
        constexpr auto parse_success(OutType parsed, std::string_view remaining) {
            return parse_result<OutType>{true, false, std::move(parsed), remaining};
        }
        ...
        constexpr auto parse_manual_unsafe(bool success, OutType parsed, std::string_view remaining) {
            return parse_result<OutType>{success, false, std::move(parsed), remaining};
        }
        ...
        struct optional_parser {
            ...
            static auto parse(std::string_view input, parse_context& ctx) {
                ...
                if (result.success) {
                    return parse_manual_unsafe<std::optional<ElementType>>(true, (std::move(result.parsed)), result.remaining);
                } else {
        ...
        struct commit_parser {
            ...
            static auto parse(std::string_view input, parse_context& ctx) {
                ...
                auto res = parse_manual_unsafe<typename BaseParser::value_type>(true, std::move(result.parsed), result.remaining);
                res.committed = true;
                return res;
            }
        };
        ...
        struct many_parser {
            ...
            static auto parse(std::string_view input, parse_context& ctx) {
                ...
                return parse_manual_unsafe<std::vector<ElementType>>(true, std::move(results), remaining);
            }
        };
        ...
        struct fmap_parser {
            static auto parse(std::string_view input, parse_context& ctx) {
                auto result = BaseParser::parse(input, ctx);
                using mapped_type = decltype(apply_fmap(std::move(result.parsed)));
                if (!result.success) {
                    return parse_failure<mapped_type>(input);
                }
                return parse_success<mapped_type>(apply_fmap(std::move(result.parsed)), result.remaining);
            }
        ...
        struct choice_parser {
            using value_type = details::static_variant<typename SubParsers::value_type...>;
            static auto parse(std::string_view input, parse_context& ctx) {
                ...
                template for (constexpr int i : std::define_static_array(details::make_index_array<sizeof...(SubParsers)>())) {
                    ...
                    if (succ) {
                        ...
                        return parse_success<ParserOutputs>(std::move(parsed_value), remaining);
                    }
                ...
                return parse_failure<details::static_variant<typename SubParsers::value_type...>>(input);
            }
        };
            ...
            static auto parse(std::string_view input, parse_context& ctx) {
                ...
                template for (constexpr int i : std::define_static_array(details::make_index_array<sizeof...(SubParsers)>())) {
                    ...
                    if constexpr (target_idx != -1) {
                        std::get<target_idx>(result_tuple) = std::move(parsed);
                    }

                    parse_remaining = remaining;
                }

                return parse_success<ReturnTupleType>(std::move(result_tuple), parse_remaining);
            }
        };
